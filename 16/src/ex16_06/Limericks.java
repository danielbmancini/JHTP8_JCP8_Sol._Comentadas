/*
 * A more efficient way to this kind of operation would be by using ML to vectorize different limericks, in terms
...
public class Limericks {
    ...
    private static int[][] limerickStructure_2 = {{8, 7, 2, 1, 10, 0, 10},
            {9, 3, 11, 6, 2, 0, 10},
            {8, 2, 1, 10},
            {7, 2, 7, 10},
            {7, 1, 10, 2, 8, 9, 1, 10}};

    private static int[][] limerickStructure_3 = {{8, 4, 1, 11, 10, 2, 10},
            {7, 11, 8, 2, 10},
            {8, 3, 2, 0, 1, 10},
            {5, 11, 0, 1, 10},
            {9, 7, 2, 7, 0, 10}};

    private static ArrayList<int[][]> structuresList = new ArrayList<>(3);

    static {
        structuresList.add(limerickStructure_1);
        structuresList.add(limerickStructure_2);
        structuresList.add(limerickStructure_3);
    }

    public String generateLimerick() {
        StringBuilder limerick = new StringBuilder();
        limerick.append("\n");
        int structIndex = random.nextInt(3);
        int counter;
        ...
        System.out.printf("%n%nUsing structure %d.", structIndex + 1);
        for (int i = 0; i < 5; i++) { //loops through the 5 arrays an limerick structure
            counter = 0;
            limerick.append("\n");
            for (int k : structuresList.get(structIndex)[i]) {
                if (counter != structuresList.get(structIndex)[i].length - 1) {
                ...
                } else if (i == 0 || i == 1 || i == 4) //to order the rhyming scheme of limericks (AABBA)
